...
async function load(){
  const [st,auto,ov,cur,adv,en]=await Promise.all([
  ...
  apps=parseTsv(auto).map(x=>({pkg:x[0],base:x[1]||'balanced',reason:x[2]||'',seen:x[3]||''})).filter(x=>x.pkg);
  overrides=new Map(parseTsv(ov).filter(x=>x[0]&&x[1]).map(x=>[x[0],x[1]]));
  $('count').textContent=String(apps.length);$('advanced').value=adv;
  try{for(let i=0;i<apps.length;i+=80){const info=getPackagesInfo(apps.slice(i,i+80).map(x=>x.pkg))||[];for(const p of info)if(p&&p.packageName)labels.set(p.packageName,p.appLabel||p.packageName)}}catch(_){}
...
function renderApps(){const q=$('search').value.trim().toLowerCase();const root=$('apps');root.innerHTML='';const list=apps.filter(a=>(labels.get(a.pkg)||a.pkg).toLowerCase().includes(q)||a.pkg.toLowerCase().includes(q));if(!list.length){root.innerHTML='<p>还没有记录。打开任意应用后会自动出现。</p>';return}for(const a of list){const row=document.createElement('div');row.className='app';const info=document.createElement('div');const n=document.createElement('strong');n.textContent=labels.get(a.pkg)||a.pkg;const sm=document.createElement('small');sm.textContent=`${a.pkg} · 自动: ${a.base} · ${a.reason}`;info.append(n,sm);const sel=document.createElement('select');const o0=document.createElement('option');o0.value='';o0.textContent=`自动 (${a.base})`;sel.append(o0);for(const p of profiles){const o=document.createElement('option');o.value=p;o.textContent=p;sel.append(o)}sel.value=overrides.get(a.pkg)||'';sel.addEventListener('change',()=>saveOverride(a.pkg,sel.value));row.append(info,sel);root.append(row)}}
async function saveOverride(pkg,val){if(val)overrides.set(pkg,val);else overrides.delete(pkg);const text=[...overrides.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([p,v])=>`${p}\t${v}`).join('\n')+(overrides.size?'\n':'');try{await writeFile(`${MOD}/config/overrides.tsv`,text);await sh(`'${MOD}/scripts/rebuild-config.sh' && '${MOD}/scripts/auto-controller.sh' --once`);toast('策略覆盖已应用');await load()}catch(e){toast('保存失败')}}
...
$('saveAdvanced').addEventListener('click',async()=>{try{await writeFile(`${MOD}/config/advanced.kdl`,$('advanced').value);await sh(`'${MOD}/scripts/rebuild-config.sh'`);toast('高级 KDL 已保存，原版 daemon 将热加载')}catch(e){toast('保存失败')}});
$('loadKdl').addEventListener('click',async()=>{$('output').textContent=await sh(`cat '${MOD}/config/threadctl.kdl' 2>/dev/null || true`)});
$('loadLog').addEventListener('click',async()=>{$('output').textContent=await sh(`echo '=== threadctl ==='; tail -n 100 '${MOD}/run/threadctl.log' 2>/dev/null; echo; echo '=== auto ==='; tail -n 100 '${MOD}/run/auto.log' 2>/dev/null`)});
